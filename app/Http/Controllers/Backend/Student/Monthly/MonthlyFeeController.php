...
namespace App\Http\Controllers\Backend\Student\Monthly;
...
class MonthlyFeeController extends Controller
{
    public function MonthlyFeeView()
    {
        ...
        return view('backend.main-section.page.student.monthly.monthly_view',$data);
    }
    ...
    public function MonthlyFeeClassData(Request $request){
        ...
        foreach ($allStudent as $key => $v) {
            ...
            $html[$key]['tdsource'] .= '<td>'.'BDT.'.$registrationfee->amount.'</td>';
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.monthly.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&month='.$request->month.'">Invoice</a>';
            $html[$key]['tdsource'] .= '</td>';
   ...
   public function MonthlyFeePayslip(Request $request){
    ...
    $pdf = PDF::loadView('backend.main-section.page.student.monthly.monthly_detail_pdf', $data);
