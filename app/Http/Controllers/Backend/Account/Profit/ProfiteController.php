...
use App\Models\Account\Employee\AccountEmployeeSalary;
...
class ProfiteController extends Controller
{
    public function MonthlyProfitView(){
    	return view('backend.main-section.page.account.report.profit.view');
    ...
    public function MonthlyProfitDatewais(Request $request){
        ...
        $th_salary = AccountTeacherSalary::whereBetween('date',[$start_date,$end_date])->sum('amount');
    	...
    	$profit = $student_fee-$total_cost;  
         ...
         $html['thsource'] .= '<th>Teacher Salary</th>';
         ...
         $html['tdsource']  .= '<td>'.$th_salary.'</td>';
    	 $html['tdsource']  .= '<td>'.$emp_salary.'</td>';
    	 $html['tdsource']  .= '<td>'.$total_cost.'</td>';
    	 $html['tdsource']  .= '<td>'.$profit.'</td>';
    	 $html['tdsource'] .='<td>';
    	 	$html['tdsource'] .='<a class="btn btn-xs btn-'.$color.'" title="PDF" target="_blanks" href="'.route("report.profit.pdf").'?start_date='.$sdate.'&end_date='.$edate.'">Pay Slip</a>';
    	 	$html['tdsource'] .= '</td>';
    ...
    $pdf = PDF::loadView('backend.main-section.page.account.report.profit.detail', $data);
